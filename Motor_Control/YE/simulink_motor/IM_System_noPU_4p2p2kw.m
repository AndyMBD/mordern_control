...
while (t < tfinal)
    ...
    wrm_err = wrm_ref - wrm; 
%     y = y + wrm_err * Ki * dt;  % 速度誤差積分 
%     if y >= Tb*2
%         y = Tb*2;
%     elseif y <= -Tb*2
%         y = -Tb*2;
%     end
%     pis_out = Kp * wrm_err + y;  % ASR PI控制器輸出
% %     ysp = Kp * wrm_err;
% %     ysint = Ki * dt * (wrm_err + wrm_err1)/2 + ysint1;
%     
%     if pis_out > Tb*2
%         pis_out = Tb*2;
%     elseif pis_out < -Tb*2
...
%         x1 = -Ib*2;
%     end
%     piphi_out = Kp_phi * phi_err + x1;  % 磁通控制器輸出
%     piphi_out = piphi_out1 + phi_err*10.03575 - 9.96425 * phi_err1;
...
%     w = w + Ki_iq * iq_err * dt;  % 磁通誤差積分 
%     if w >= (Ib*2)/(Sigma*Ls)
%         w = Ib*2/(Sigma*Ls);
%     elseif w <= -Ib*2/(Sigma*Ls)
%         w = -Ib*2/(Sigma*Ls);
%     end
%     piiq_out = Kp_iq * iq_err + w;  % 磁通控制器輸出    
    if piiq_out >= Ib*2/(Sigma*Ls)
        piiq_out = Ib*2/(Sigma*Ls);
    elseif piiq_out <= -Ib*2/(Sigma*Ls)
        piiq_out = -Ib*2/(Sigma*Ls);
    end
%     piq_out = iq_err*105.95 - iq_err1*94.05 + piq_out1;
    ...
    Vq_ref_dep = Sigma*Ls*(Vq_ref + we*idse + (1-Sigma)/(Sigma*Lm)*we*phi_dr_est);
%     if Vq_ref_dep > 150 %相電壓最大值不超過vdc/2
%        Vq_ref_dep = 150;
%     end 
...
%     z = z + Ki_id * id_err * dt;  % 磁通誤差積分 
%     if z >= Ib*2/(Sigma*Ls)
%         z = Ib*2/(Sigma*Ls);
%     elseif z <= -Ib*2/(Sigma*Ls)
%         z = -Ib*2/(Sigma*Ls);
%     end
%     piid_out = Kp_id * id_err + z;  % 磁通控制器輸出    
    if piid_out >= Ib*2/(Sigma*Ls)
        piid_out = Ib*2/(Sigma*Ls);
    elseif piid_out <= -Ib*2/(Sigma*Ls)
        piid_out = -Ib*2/(Sigma*Ls);
    end

%     pid_out = id_err*105.95 - id_err1*94.05 + pid_out1;
    id_err1 = id_err;
    piid_out1 = piid_out;
    Vd_ref = piid_out;  % d軸電流PI輸出d軸電壓命令
    Vd_ref_dep = Sigma*Ls*(Vd_ref - we*iqse - (1-Sigma)/(Sigma*Tr*Lm)*phi_dr_est);
%     if Vd_ref_dep > 150 %相電壓最大值不超過vdc/2
%        Vd_ref_dep = 150;
%     end 
...
%     Vd_ref = Kp_id * id_err + Ki_id * z;  % d軸電流PI輸出d軸電壓命令
    %----------------------------------------------------------
    % 產生三相靜止座標電壓命令
    Vas_ref = Vd_ref_dep*cos(theta_e) - Vq_ref_dep*sin(theta_e);
    Vbs_ref = Vd_ref_dep*cos(2*pi/3 - theta_e) + Vq_ref_dep*sin(2*pi/3 - theta_e);
    Vcs_ref = Vd_ref_dep*cos(4*pi/3 - theta_e) + Vq_ref_dep*sin(4*pi/3 - theta_e);
%     %SVPWM 變頻器-------------------------------------------------
