...
% Phase-lead controller (1+aTs)/(1+Ts), a>1
phim = 70; % degree
a=(1+sin(70*pi/180))/(1-sin(70*pi/180)); 
wm = 3910; % rad/s, 對應未補償系統 -10*log10(a)的角頻率
T = 1/(sqrt(a)*wm);
G_phase_lead = tf([a*T 1], [T 1]);
Go_w_withDelay_phase_lead = 28*Go_w_withDelay*G_phase_lead;
h=bodeoptions;
h.PhaseMatching='on';
h.Title.FontSize = 14;
...
figure(2)
bodeplot(28*Go_w_withDelay,'-k', Go_w_withDelay_phase_lead, '-b',{1,10000},h);
legend('No Compensated', 'Phase-lead Compensated');
